...
#pragma omp critical(negshifts)
						{
							...
							vector<int> individuals;
							individuals[0]= dous[j]->n;

							if(dous[j]->pars[0]){
									individuals.push_back(dous[j]->pars[0]->n);
									if(dous[j]->pars[0]->pars[0]){
										individuals.push_back(dous[j]->pars[0]->pars[0]->n);
									}
									if(dous[j]->pars[0]->pars[1]){
										individuals.push_back(dous[j]->pars[0]->pars[1]->n);
									}
								}
								if(dous[j]->pars[1]){
									individuals.push_back(dous[j]->pars[1]->n);
									if(dous[j]->pars[1]->pars[0]){
										individuals.push_back(dous[j]->pars[0]->pars[0]->n);
									}
									if(dous[j]->pars[1]->pars[1]){
										individuals.push_back(dous[j]->pars[1]->pars[1]->n);
									}
								}
							...
							for (int g = 0; g < NUMTURNS; g++) {
								...
								c.weight = w;
								c.individuals = claus;
								toulInput[marker - qstart].push_back(c);
							}
...
/*							for (int side = 0; side < 2; side++)
		...
		for (int g=0; g< sizeof(toulIn) ; g++ ){//change size to what the size actually is
			toulIn[g]
		}
...
int main(int argc, char* argv[])
{
	...
	mpi::communicator world;
		...
		fputs("c This is the in file for Toulbar2", toulIn);
		fputs("Hello !!!!!!!!!!!!!!!!!", toulIn);
		//fputc('/n', toulIn);
		fputs(
